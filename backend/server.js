...
// For Vercel serverless, we need to handle things differently
// since Socket.io doesn't work well with serverless functions
...
if (isVercel) {
  ...
  // Note: Socket.io won't work properly in serverless environment
  
  // Simple HTTP-only mode for Vercel
  const server = http.createServer(app);
  ...
  // Only set up Socket.io for non-Vercel environments
  export const io = new IOServer(server, {
    cors: { origin: "*" },
  });
