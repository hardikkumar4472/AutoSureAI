...
import { ClipboardList, Clock, CheckCircle, XCircle, TrendingUp, Shield, Activity, AlertTriangle } from 'lucide-react';
...
const TrafficAnalytics = () => {
    ...
    const fetchReports = async () => {
        try {
            const response = await api.get('/traffic/reports');
    ...
    const stats = {
        ...
        severe: reports.filter(r => r.prediction?.severity === 'severe').length,
        moderate: reports.filter(r => r.prediction?.severity === 'moderate').length,
    ...
    const getTimelineData = () => {
        ...
        reports.forEach(report => {
            ...
            if (!reportsByDate[date]) {
                reportsByDate[date] = { date, pending: 0, verified: 0, fraudulent: 0, total: 0 };
            }
            reportsByDate[date].total++;
            if (report.verification?.status === 'unverified') {
                reportsByDate[date].pending++;
            } else if (report.verification?.status === 'verified') {
                reportsByDate[date].verified++;
            } else if (report.verification?.status === 'fraudulent') {
                reportsByDate[date].fraudulent++;
            }
        });
            ...
            <div className="max-w-7xl mx-auto space-y-8">
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    <div className="space-y-2">
                        ...
                        <p className="text-lg text-gray-600 dark:text-gray-300 font-light">
                            Detailed insights and trends for accident reports
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <StatCard
                        ...
                        description="All accident reports"
                    />
                    <StatCard
                    ...
                    <StatCard
                        ...
                        description="Confirmed accidents"
                    />
                    <StatCard
                        title="Fraudulent Reports"
                ...
                {reports.length === 0 ? (
                    <div className="card rounded-3xl p-12 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-xl text-center">
                        ...
                        <p className="text-gray-600 dark:text-gray-300 max-w-md mx-auto">
                            No accident reports found in the system.
                        </p>
                    </div>
                ) : (
                    <>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div className="card rounded-3xl p-8 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-xl">
                                <div className="flex items-center space-x-3 mb-6">
                                    ...
                                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Reports by Status</h2>
                                </div>
                            ...
                            {timelineData.length > 0 && (
                                <div className="card rounded-3xl p-8 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-xl lg:col-span-2">
                                    <div className="flex items-center space-x-3 mb-6">
                                        <div className="w-2 h-8 bg-gradient-to-b from-blue-600 to-blue-400 rounded-full"></div>
                                        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Reports Timeline</h2>
                                    </div>
                                    <ResponsiveContainer width="100%" height={300}>
                                        <AreaChart data={timelineData}>
                                            ...
                                            <Area
                                                ...
                                                name="Total Reports"
                                            />
                                            <Line type="monotone" dataKey="verified" stroke="#22c55e" strokeWidth={2} name="Verified" dot={false} />
                                            <Line type="monotone" dataKey="fraudulent" stroke="#ef4444" strokeWidth={2} name="Fraudulent" dot={false} />
                                        </AreaChart>
                                    </ResponsiveContainer>
                                </div>
                            )}
